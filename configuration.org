...
(use-package dashboard
  ...
  (progn
    ...
    (setq dashboard-startup-banner "~/.emacs.d/images/yorha.png")
    )
  :config
...
** markup+
*** web-mode
This mode supports multiple "languages" when there is no native mode for them.
...
*** vue
When a =.vue= file is opened in a project vue-semantic-server is used for all files, which breaks development for =astro= projects and other ones that would have Vue in them
...
(use-package c-ts-mode
  :mode ("\\.c\\'")
  :hook (c-ts-mode .(lambda () (setq tab-width 4))))
#+end_src
* Org-mode
** General config
#+begin_src emacs-lisp
